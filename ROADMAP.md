...
| 2026-09-24 | **PDF text selection, interactive gesture overlay, highlight toolbar, and offline dictionary lookup.** (1) **Native MuPDF Structured Text Extraction**: Added `PdfTextChar`, `PdfTextLine`, and `PdfPageText` in `src/pdf.rs` wrapping MuPDF's `Page::to_text_page` and `StructuredText`. Extracted exact vector glyph quads and bounding boxes in PDF points (72 DPI) with reading-order string reconstruction and line selection. (2) **Hardware-Accelerated Cairo Selection Overlays**: Layered `gtk::Overlay` and transparent `gtk::DrawingArea` over page pictures across all 4 scroll modes (`Page Scrolling`, `Vertical Scrolling`, `Horizontal Scrolling`, `Wrapped Scrolling`) and spread modes (`No Spreads`, `Odd Spreads`, `Even Spreads`). Connected `gtk::GestureDrag` for responsive drag selection with sub-pixel coordinate mapping from screen pixels to document points. Highlighted selections render with smooth translucent blue boxes (`rgba(53, 132, 228, 0.35)`). (3) **Unified Selection Popover Chip**: Displayed a floating toolbar popover anchored directly above the selected text containing 5 color highlight swatches (yellow, green, blue, pink, orange), Define in Dictionary (`D`), and Copy to Clipboard (`Ctrl+C`). (4) **Offline Dictionary Popover & Annotations**: Built a popup card showing dictionary definition, pronunciation, part of speech badge, and examples via `catalog.lookup_entry()`. Integrated color highlights into catalog annotations database (`insert_annotation`). Added keyboard bindings (`Ctrl+C` for copy, `D` for define, `Escape` to dismiss selection). |
| 2026-09-24 | **PDF reader layout & gesture regressions resolved.** Root cause analysis revealed that adding `gtk::Overlay` with `gtk::DrawingArea` (`hexpand: true`, `vexpand: true`) over page pictures broke GTK4 layout sizing across all scroll modes, stretching containers to 100% viewport dimensions, breaking natural page aspect ratios, displacing loading spinners, and creating ghost overlays in Two-Page Spreads (where cover page 1 had an invisible right overlay occupying 50% width). Furthermore, gesture controllers on drawing areas conflicted with the viewport click controller and forced an I-beam cursor across the whole screen. Reverted `src/pages/pdf_reader.rs` to the clean, proven architecture from `bc8b45a`: direct `gtk::Picture` rendering with native MuPDF rasterization, perfect centering in single and continuous modes, centered single cover in Odd Spreads, smooth in-place zoom resizing without layout distortion, normal arrow cursor, and reliable autohide edge chrome. MuPDF backend text extraction (`src/pdf.rs`) preserved independently with `#[allow(dead_code)]` for clean downstream integration. |
| 2026-09-24 | **EPUB reader line cut-off resolved & PDF unified chrome restored.** (1) **EPUB reader line clipping fixed**: Reverted the 12px bleed offset on `blit_rows` in `crates/kalam-reader/src/view.rs`. The 12px upward destination shift caused subsequent bands in continuous and paginated strips to blit over the bottom 12px of preceding bands, horizontally slicing lines of body text in half across page seams. (2) **PDF bottom pill transparency fixed**: Unified bottom pill markup with `kalam-reader-bottom-dock` and `kalam-reader-bottom-pill` in `src/pages/pdf_reader.rs`, added `.kalam-reader-floating-pill` alias and `.kalam-reader-pill-sep` rules in `resources/style.css`, restoring solid `@kalam_surface` background, rounded border, and shadow. (3) **Top dock styling & sidebar button restored**: Replaced unstyled back chip with `kalam-reader-back` pill containing `Library` (with `go-previous-symbolic` icon), dedicated `view-sidebar-start-symbolic` sidebar toggle, and `bookmark-new-symbolic` bookmark toggle matching EPUB reader. (4) **Sidebar access & stability fixed**: Added `kalam-reader-dim` overlay backdrop so clicking outside closes the sidebar cleanly; added close timer cancellation upon mouse enter into the sidebar or explicit toggle, preventing premature auto-close; initialized TOC tab as active. |
| 2026-09-24 | **PDF reader chrome regression audit and architecture restoration.** (1) **Root cause diagnosis**: Traced git history across `c623caa`, `a2e5f92`, `3ed52e1`, and `e9a84a0`. `a2e5f92` had introduced full-width 45px overlay boxes (`kalam-reader-edge-strip-top/bottom`) that intercepted clicks across the viewport, stripped the left edge hover box, replaced the `.kalam-reader-bottom-pill` markup with unstyled `.kalam-reader-floating-pill` (causing total transparency), replaced `kalam-reader-back` with an unstyled `kalam-reader-back-chip`, and removed the dim backdrop. (2) **Unified Reader Chrome Restored**: Rebuilt overlay hierarchy in `src/pages/pdf_reader.rs` matching `c623caa` and EPUB reader: Overlay 1 ScrolledWindow viewport; Overlay 2 `kalam-reader-dim` backdrop; Overlay 3 dedicated 18px `left_edge_hover` strip; Overlay 4 floating `back_dock` with clean `kalam-reader-back` "Library" pill; Overlay 5 floating `bottom_dock` (`kalam-reader-bottom-dock` containing solid `@kalam_surface` `.kalam-reader-bottom-pill` with sidebar toggle, prev/next, page counter, bookmark toggle, and zoom controls); Overlay 6 slide-in left sidebar (TOC, Bookmarks, Settings). (3) **Sidebar Hover & Pinning Stability**: Attached dedicated motion controller to `widgets.left_edge_hover` and removed interfering `x < 25.0` check on root; added `sidebar_pinned` state so explicitly opening the sidebar via button or `t` key keeps it open without mouse-leave auto-closing; added active styling for bookmark indicator; clicking dim backdrop or pressing Esc closes and unpins sidebar cleanly. |
