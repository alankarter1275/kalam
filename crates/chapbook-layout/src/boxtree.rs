...
//! in flow. `::before`/`::after` emit string, `attr()`, and quote content
//! (`counter()`/`counters()` and images in `content` are skipped).
//!
//! kalam: a replaced element (an image, a rule, a rasterized `<svg>`, a
//! formula) reached only through inline wrappers — `<a><img/></a>`,
//! `<span class="squeeze"><span><img/></span></span>` — is hoisted into
//! its own block exactly as a direct child is, by treating every inline
//! wrapper on the way down as block-level (see [`has_block_child`] and
//! [`wraps_replaced`]). Upstream skipped anything nested deeper than one
//! level, which dropped every full-page image of a Penguin Random House
//! book (their images sit two `inline-block` divs deep). Text beside the
//! image inside the same wrapper becomes an anonymous block above or
//! below it, so the inline-in-block flattening upstream already applied
//! to `<p><img/>text</p>` is what happens at any depth.
...
fn build_block(input: &BoxTreeInput, node: NodeId, style: ServoArc<ComputedValues>) -> BlockBox {
    ...
    let kind = if has_block_child(input, node) {
...
/// The content model of `node`: any block-level element child makes it a
/// container of blocks (with anonymous blocks around the inline runs);
/// otherwise it is one inline formatting context.
///
/// Replaced children (img/hr) count as block-level — they always hoist
/// to their own block, so `<p><img/>text</p>` becomes a container instead
/// of dropping the image (and a floated image can wrap its paragraph's
/// text). kalam: so does an inline child that merely *wraps* a replaced
/// element (`<a><img/></a>`, a publisher's `inline-block` sizing div);
/// see [`wraps_replaced`].
fn has_block_child(input: &BoxTreeInput, node: NodeId) -> bool {
    let doc = input.doc;
    doc.node(node).children.iter().any(|child| {
        if !matches!(&doc.node(*child).data, NodeData::Element(_)) {
            return false;
        }
        if replaced_kind(input, *child).is_some() {
            return true;
        }
        ...
        match display_of(&style) {
            DisplayClass::None => false,
            DisplayClass::Inline => wraps_replaced(input, *child),
            DisplayClass::Block | DisplayClass::ListItem => true,
        }
    })
}

/// kalam: whether an inline element holds, somewhere below it, a replaced
/// element that would actually be placed (an image the store decoded, a
/// rule, a prepared formula). Such a wrapper is laid out as a block so
/// the replaced element hoists out of it the way it hoists out of a
/// paragraph; an inline wrapper holding text alone — or an image that
/// failed to decode — stays in its line, as it always did. Hidden
/// subtrees are not looked into: a `display: none` image is no reason to
/// break the wrapper's text out of its line.
fn wraps_replaced(input: &BoxTreeInput, node: NodeId) -> bool {
    let doc = input.doc;
    doc.node(node).children.iter().any(|child| {
        if !matches!(&doc.node(*child).data, NodeData::Element(_)) {
            return false;
        ...
        replaced_present(input, *child) || wraps_replaced(input, *child)
    })
}

/// Whether [`replaced_block`] would build a block for `id`: a replaced
/// element whose content is at hand.
fn replaced_present(input: &BoxTreeInput, id: NodeId) -> bool {
    match replaced_kind(input, id) {
        Some(ReplacedKind::Image) => input.images.dims(crate::dom::node_tag(id)).is_some(),
        Some(_) => true,
        None => false,
    }
}
    ...
    for child in &doc.node(node).children {
        match &doc.node(*child).data {
            ...
            NodeData::Element(_) => {
                ...
                match display_of(&style) {
                    DisplayClass::None => {}
                    // kalam: an inline wrapper around a replaced element is
                    // laid out as a block (`has_block_child`), so the
                    // element inside hoists out of it at the next level.
                    DisplayClass::Inline if !wraps_replaced(input, *child) => {
                        collect_inline_element(input, *child, style, pending_inline);
                    }
                    DisplayClass::Inline | DisplayClass::Block | DisplayClass::ListItem => {
                        flush_anonymous(
                            input,
                            children,
                            pending_inline,
