...
// ---- kalam: replaced elements inside inline wrappers ----

/// The image fragments on a page, in placement order.
fn image_rects(layout: &ChapterLayout, page: usize) -> Vec<chapbook_core::Rect> {
    layout.pages[page]
        .fragments
        .iter()
        .filter(|f| matches!(f.kind, FragmentKind::Image { .. }))
        .map(|f| f.rect)
        .collect()
}

#[test]
fn an_image_two_inline_wrappers_deep_is_still_placed() {
    // Penguin Random House's full-page image: `div.squeeze.center >
    // div.squeeze.squeeze95 > img`, `.squeeze` being `inline-block`. The
    // cover of the same book has one wrapper and always rendered; this
    // shape rendered nothing.
    let html = "<html><body><div>\
                <div class=\"squeeze center\"><div class=\"squeeze squeeze95\">\
                <img src=\"part.jpg\" alt=\"Part I\"/></div></div>\
                </div></body></html>";
    let (layout, _) = layout_html_with_image(
        html,
        ".squeeze { display: inline-block; max-width: 100%; } \
         .center { text-align: center; } .squeeze95 { width: 95%; }",
        &page_for_lines(12),
        (100, 54),
    );
    let images = image_rects(&layout, 0);
    assert_eq!(images.len(), 1, "nested image not placed: {images:?}");
    // Centered in the measure like any hoisted image, at the content top.
    assert_eq!(images[0].size, Size::new(100.0, 54.0));
    assert!((images[0].origin.x - (40.0 + (520.0 - 100.0) / 2.0)).abs() < 0.5);
    assert!((images[0].origin.y - 40.0).abs() < 0.5);
}

#[test]
fn an_image_inside_a_link_inside_a_span_is_placed_and_the_text_flows_around() {
    // `<a><img/></a>` is how many books link a cover or a figure; here it
    // is wrapped once more and has words on either side, which must land
    // above and below the picture, in order, none lost.
    let html = "<html><body><p>Before <span><a href=\"x.xhtml\">\
                <img src=\"fig.png\" alt=\"figure\"/></a></span> after</p></body></html>";
    let (layout, _) =
        layout_html_with_image(html, "p { margin: 0; }", &page_for_lines(12), (100, 54));
    let images = image_rects(&layout, 0);
    assert_eq!(images.len(), 1, "image lost: {images:?}");
    let lines = line_frags(&layout);
    let texts: Vec<&str> = lines.iter().map(|l| l.3.as_str()).collect();
    assert_eq!(texts, vec!["Before", "after"], "text order: {lines:?}");
    let (before, after) = (&lines[0], &lines[1]);
    assert!(
        before.1 + 27.0 <= images[0].origin.y + 0.5,
        "text before sits above: {before:?}"
    );
    assert!(
        after.1 >= images[0].origin.y + 54.0 - 0.5,
        "text after sits below: {after:?}"
    );
}

#[test]
fn an_inline_wrapper_with_only_text_stays_in_its_line() {
    // The other side of the rule: `inline-block` spans with text and no
    // image are still inline — one line, not three blocks.
    let html = "<html><body><p>One <span class=\"box\">two</span> three</p></body></html>";
    let (layout, _) = layout_html_with_image(
        html,
        "p { margin: 0; } .box { display: inline-block; }",
        &page_for_lines(12),
        (100, 54),
    );
    let lines = line_frags(&layout);
    assert_eq!(lines.len(), 1, "{lines:?}");
    assert_eq!(lines[0].3, "One two three");
}

#[test]
fn a_wrapped_image_that_did_not_decode_leaves_the_line_alone() {
    // No image bound to the `<img>` (an unresolvable src): the wrapper is
    // ordinary inline text, exactly as before, and nothing is placed.
    let html = "<html><body><p>One <span><img src=\"missing.png\"/>two</span> three</p>\
                </body></html>";
    let (layout, _) = layout_html(html, "p { margin: 0; }", &page_for_lines(12));
    assert!(image_rects(&layout, 0).is_empty());
    let lines = line_frags(&layout);
    assert_eq!(lines.len(), 1, "{lines:?}");
    assert_eq!(lines[0].3, "One two three");
}
...
#[test]
fn a_hidden_wrapped_image_leaves_the_line_alone() {
    let html = "<html><body><p>One <span><img src=\"x.png\" class=\"hide\"/>two</span> three</p>\
                </body></html>";
    let (layout, _) = layout_html_with_image(
        html,
        "p { margin: 0; } .hide { display: none; }",
        &page_for_lines(12),
        (100, 54),
    );
    assert!(image_rects(&layout, 0).is_empty());
...
#[test]
fn a_floated_image_inside_a_link_still_floats() {
    // The float tests above use `<p><img/>text</p>`; the same picture
    // behind a link must reach `place_float_image` too.
    let words = "wrap ".repeat(60);
    let html = format!(
        "<html><body><p><a href=\"big.png\"><img src=\"x.png\"/></a>{words}</p></body></html>"
    );
    let (layout, _) = layout_html_with_image(
        &html,
        "p { margin: 0; } img { float: left; margin: 0 10px 10px 0; }",
    ...
    assert_eq!(images[0].origin, chapbook_core::Point::new(40.0, 40.0));
    let lines = line_frags(&layout);
    assert!(
        lines.iter().filter(|l| l.0 > 145.0).count() >= 2,
        "expected shortened lines beside the float: {lines:?}"
    );
}
