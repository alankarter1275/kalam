...
/// box, in CSS px — reduced so adjacent lines have distinct ribbons.
pub const BAND_PADDING: f32 = 0.5;
/// Minimum vertical space left between adjacent selection lines (CSS px).
pub const MIN_LINE_GAP: f32 = 3.5;
/// Additional trim from top of the band to snug against capital letters.
pub const TOP_INSET: f32 = 2.5;

impl LineFragment {
    /// kalam: the vertical extent of a selection band on this line, as
    /// (top, bottom) offsets from the fragment top: trimmed from the top
    /// to sit close to uppercase glyph tops while fixing the bottom and
    /// keeping at least [`MIN_LINE_GAP`] between consecutive lines.
    pub fn band_extent(&self, line_height: f32) -> (f32, f32) {
        let half_gap = (MIN_LINE_GAP / 2.0).min(line_height * 0.15);
        let min_top = half_gap;
        let max_bottom = (line_height - half_gap).max(min_top + 1.0);

        if self.ascent + self.descent <= 0.0 {
            let h = (line_height * 0.65).min(line_height - MIN_LINE_GAP);
            let bottom = (line_height - half_gap).min(max_bottom);
            let top = (bottom - h).max(min_top);
            return (top, bottom);
        }
        let bottom = (self.baseline + self.descent + BAND_PADDING).min(max_bottom);
        let top = (self.baseline - self.ascent + TOP_INSET)
            .min(bottom - 4.0)
            .max(min_top);
        (top, bottom)
    }
...
mod tests {
    ...
    fn band_is_the_glyph_box_plus_padding_inside_the_line_box() {
        ...
        assert!(top >= 1.75, "top {top} leaves gap");
        assert!(bottom <= 30.6 - 1.75, "bottom {bottom} leaves gap");
        assert!(bottom > top);
    }
    ...
    fn band_never_leaves_the_line_box() {
        ...
        let (top, bottom) = l.band_extent(21.0);
        assert!(top >= 1.75 && bottom <= 19.25);
        let l = line(0.0, 0.0, 0.0);
        let (top, bottom) = l.band_extent(10.0);
        assert!(top >= 1.0 && bottom <= 9.0);
    }
}
