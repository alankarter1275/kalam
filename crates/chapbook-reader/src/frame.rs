...
impl Session {
    ...
    pub(crate) fn page_display_list(&mut self) -> Option<chapbook_paint::DisplayList> {
        self.metrics?;
        ...
        // the same unit, borrowed together. kalam: the store decides how
        // each image meets the ground (paper inverts on a dark page).
        let unit = self.units.get(&spine)?;
        let page = unit.layout.as_ref()?.pages.get(page_idx)?;
        let images = unit.images.as_ref().unwrap_or(&self.empty_images);
        let mut list = chapbook_paint::build_display_list(page, background, &selections, images);
