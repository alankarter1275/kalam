...
impl Session {
    ...
    pub fn page_frame(&mut self, spine: usize, page: usize) -> Option<DisplayList> {
        ...
        // Field accesses: the layout and the image store are two fields of
        // the same unit, borrowed together.
        let unit = self.units.get(&spine)?;
        let page = unit.layout.as_ref()?.pages.get(page)?;
        let images = unit.images.as_ref().unwrap_or(&self.empty_images);
    ...
    /// The paragraph under a point on any page, as a locator range.
    pub fn paragraph_at_page(
        &mut self,
        spine: usize,
        page_idx: usize,
        x: f32,
        ...
        let mut hit_tag = None;
        let mut hit_range = None;
        for fragment in &page.fragments {
            let (chapbook_paint::FragmentKind::Line(line) | chapbook_paint::FragmentKind::HiddenText(line)) = &fragment.kind else {
                continue;
            };
            let r = fragment.rect;
            if pt.y >= r.origin.y && pt.y <= r.origin.y + r.size.h {
                hit_tag = Some(fragment.tag);
                let line_start = line.runs.iter().flat_map(|r| r.glyphs.iter()).map(|g| g.locator).min().unwrap_or(line.locator_start);
                let line_end = line.runs.iter().flat_map(|r| r.glyphs.iter()).map(|g| g.locator + 1).max().unwrap_or(line.locator_start + line.text.chars().count() as u32);
                hit_range = Some((line_start, line_end));
                break;
            }
        }
        let tag = hit_tag?;
        if tag != 0 {
            ...
            for p in &layout.pages {
                for fragment in &p.fragments {
                    if fragment.tag == tag {
                        if let chapbook_paint::FragmentKind::Line(line) | chapbook_paint::FragmentKind::HiddenText(line) = &fragment.kind {
                            let s = line.runs.iter().flat_map(|r| r.glyphs.iter()).map(|g| g.locator).min().unwrap_or(line.locator_start);
                            let e = line.runs.iter().flat_map(|r| r.glyphs.iter()).map(|g| g.locator + 1).max().unwrap_or(line.locator_start + line.text.chars().count() as u32);
                            min_loc = min_loc.min(s);
                            max_loc = max_loc.max(e);
                            found = true;
                        }
                    }
                }
    ...
    pub fn select_word_at_page(
        ...
        let Some((start, end)) = self.word_at_page(spine, page, x, y) else {
            return false;
        };
    ...
    /// Select the paragraph under a point on any page.
    pub fn select_paragraph_at_page(
        &mut self,
        spine: usize,
        page: usize,
        x: f32,
        ...
        let Some((start, end)) = self.paragraph_at_page(spine, page, x, y) else {
            return false;
        };
        self.set_position(spine, page);
        self.select_range(start, end);
    ...
    /// An image under a point on any page — [`Session::image_at`] for a
    /// page that need not be current.
    pub fn image_at_page(&mut self, spine: usize, page: usize, x: f32, y: f32) -> Option<(u32, u32, Vec<u8>)> {
        let pt = Point::new(x, y);
        let bg = self.settings.theme.background();
        let _ = self.layout_unit(spine)?;
        let unit = self.unit(spine)?;
        let layout = unit.layout.as_ref()?;
        let page_obj = layout.pages.get(page)?;
        let images = unit.images.as_ref().unwrap_or(&self.empty_images);
        let dl = chapbook_paint::build_display_list(page_obj, bg, &[], images);
        for op in &dl.ops {
            if let chapbook_paint::DisplayOp::Image { resource, dest, .. } = op {
                if dest.contains_point(pt) {
                    if let Some(stored) = images.get(*resource) {
