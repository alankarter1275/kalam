...
impl Session {
    ...
    pub fn selection_blend(&self) -> chapbook_paint::Blend {
        if chapbook_paint::is_dark_ground(self.settings.palette().background) {
    ...
    /// Locator range of the paragraph under a point in panel coordinates.
    pub fn paragraph_at(&mut self, x: f32, y: f32) -> Option<(u32, u32)> {
        let (cx, cy) = self.content_point(x, y);
        let spine = self.spine;
        let page = self.page;
        self.paragraph_at_page(spine, page, cx, cy)
    }

    /// Select the paragraph under a point in panel coordinates.
    pub fn select_paragraph_at(&mut self, x: f32, y: f32) -> bool {
        let Some((start, end)) = self.paragraph_at(x, y) else {
            return false;
        };
        self.select_range(start, end);
        true
    ...
    pub fn cached_unit_text(&self, spine: usize) -> Option<String> {
