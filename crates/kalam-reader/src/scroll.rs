...
//!
//! Everything is drawn on the device-pixel grid. A touchpad scrolls by
//! fractions of a pixel, a page's measured height is rarely a whole one,
//! and a strip that rounded each band's position separately let two
//! neighbouring bands round different ways: as the scroll passed through
//! the fractions, the text below a page seam shifted a pixel against the
//! text above it and back — a shimmer under the reader's eyes, on every
//! screen that showed the end of one page and the start of the next. So
//! every band edge the strip computes is snapped to the grid (a measured
//! page's height rounded up, so no row of it is lost; the gap before it
//! and a guessed chapter's height to the nearest pixel), and a frame is
//! drawn from [`Strip::view_top`], the scroll position snapped once for
//! the whole frame. Between two frames every band then moves by the same
//! whole number of pixels. The raw scroll keeps its fractions, so slow
//! touchpad motion still adds up to a pixel instead of being rounded
//! away event by event.
//!
//! No GTK in here. Strip coordinates are CSS px from the top of the book;
//! the widget maps them to its own (`widget y = strip y − view_top`).
//! That keeps the arithmetic testable on a machine with no display, which
...
pub(crate) struct Strip {
    ...
    generation: u64,
    px_per_char: f32,
    /// Device pixels per CSS px — the grid every band edge and the frame
    /// origin are snapped to (module docs).
    scale: f32,
}
...
impl Strip {
    ...
    /// `scale` is the widget's device pixels per CSS px.
    pub(crate) fn new(
        chars: Vec<u64>,
        viewport: f32,
        scale: f32,
    ...
    /// The scroll position as the reader set it, fractions and all — what
    /// the scrollbar shows and the anchor is taken from.
    pub(crate) fn scroll_y(&self) -> f32 {
        self.scroll_y
    }

    /// The strip y at the top edge of the frame as *drawn*: the scroll
    /// position snapped to the device-pixel grid, once for the whole
    /// frame. Every strip↔widget mapping goes through this, so what is
    /// painted, hit-tested and handled agrees to the pixel.
    pub(crate) fn view_top(&self) -> f32 {
        snap(self.scroll_y, self.scale)
    }
    ...
    pub(crate) fn rebuild(&mut self, generation: u64, viewport: f32, scale: f32) {
        self.generation = generation;
        self.viewport = viewport.max(1.0);
        self.scale = grid_scale(scale);
    ...
    /// The pages' heights go onto the pixel grid here (module docs): a
    /// band's height up to the next pixel, the gap before it to the
    /// nearest.
    pub(crate) fn measure(&mut self, spine: usize, mut pages: Vec<PageExtent>) {
        let scale = self.scale;
        for page in &mut pages {
            page.gap_before = snap(page.gap_before, scale);
            page.used_height = snap_up(page.used_height, scale);
        }
    ...
    /// Clamped to the strip; the anchor follows. Whether the *frame*
    /// moved: a change too small to reach the next device pixel keeps
    /// its fraction for the next event and needs no redraw.
    pub(crate) fn set_scroll(&mut self, y: f32) -> bool {
        let before = self.view_top();
        self.scroll_y = y.clamp(0.0, self.max_scroll());
        self.anchor = self.anchor_at(self.scroll_y + MARGIN_TOP);
        self.view_top() != before
    }
    ...
    pub(crate) fn visible_units(&self) -> std::ops::Range<usize> {
        ...
        let mut first = self.slot_at(top);
        // A viewport whose top edge is in the seam after a chapter shows
        // none of that chapter.
        let past_first = self
            .slots
            .get(first)
            .is_some_and(|slot| top >= slot.top + slot.height);
        if past_first && first + 1 < self.slots.len() {
            first += 1;
        }
        let last = self.slot_at(top + self.viewport).max(first);
        first..last + 1
    }
    ...
    pub(crate) fn visible_bands(&self) -> Vec<Band> {
        let (lo, hi) = (self.view_top(), self.view_top() + self.viewport);
        self.visible_units()
    ...
    pub(crate) fn visible_dividers(&self) -> Vec<(usize, f32)> {
        ...
        // any of it shows.
        let lo = self.view_top() - DIVIDER_BAND;
        let hi = self.view_top() + self.viewport + DIVIDER_BAND;
        self.slots
            .iter()
            .enumerate()
            .skip(1)
    ...
    pub(crate) fn to_widget(&self, band: &Band, rect: Rect) -> Rect {
        Rect::new(
            rect.origin.x,
            rect.origin.y - band.page_y + band.top - self.view_top(),
            rect.size.w,
            rect.size.h,
        )
    }
    ...
    fn reflow(&mut self) {
        ...
        for slot in &mut self.slots {
            slot.height = match &slot.pages {
                ...
                None => snap((slot.chars as f32 * density).max(floor), scale),
            };
            slot.top = y;
...
/// A usable grid from whatever the widget reported: GTK's scale factor
/// is a positive integer, but the strip must not divide by anything else.
fn grid_scale(scale: f32) -> f32 {
...
/// `v` moved to the nearest device pixel, in CSS px.
fn snap(v: f32, scale: f32) -> f32 {
    (v * scale).round() / scale
}

/// `v` moved up to the next device pixel — a height, so no row is cut.
fn snap_up(v: f32, scale: f32) -> f32 {
    (v * scale).ceil() / scale
}
...
mod tests {
    ...
    fn a_measurement_never_moves_the_text_under_the_reading_line() {
        ...
        let mut strip = Strip::new(vec![1000, 1000, 1000], VIEWPORT, 1.0, 0.5, 1);
    ...
    fn a_jump_puts_the_page_top_on_the_reading_line() {
        let mut strip = Strip::new(vec![1000; 4], VIEWPORT, 1.0, 0.5, 1);
    ...
    fn points_map_between_the_widget_and_the_page() {
        let mut strip = Strip::new(vec![4000, 4000], VIEWPORT, 1.0, 0.5, 1);
    ...
    fn the_scroll_stays_inside_the_strip() {
        let mut strip = Strip::new(vec![1000; 3], VIEWPORT, 1.0, 0.5, 1);
        ...
        let mut short = Strip::new(vec![100], VIEWPORT, 1.0, 0.5, 1);
    ...
    fn dividers_sit_in_the_seams_the_viewport_shows() {
        ...
        let mut tall = Strip::new(vec![1000; 3], 5000.0, 1.0, 0.5, 1);
        for spine in 0..3 {
    ...
    #[test]
    fn fractional_scrolling_moves_every_band_by_the_same_whole_pixel() {
        // Pages of 21 lines at 30.6 px and gaps of 3.4 px: nothing the
        // engine measures is a whole pixel. A touchpad's 7.3 px steps
        // are not either. Every frame must still put every band on the
        // grid, and every band on two consecutive frames must have moved
        // by the same whole number of pixels — the shimmer test.
        let mut strip = Strip::new(vec![4000; 3], VIEWPORT, 1.0, 0.5, 1);
        for spine in 0..3 {
            strip.measure(spine, vec![extent(0.0, 642.6, 0), extent(3.4, 428.4, 2000)]);
        }
        let frame = |strip: &Strip| -> Vec<((usize, usize), f32)> {
            let top = strip.view_top();
            assert_eq!(top.fract(), 0.0, "the frame's origin is a whole pixel");
            strip
                .visible_bands()
                .iter()
                .map(|band| {
                    let y = band.top - top;
                    assert_eq!(y.fract(), 0.0, "band {:?} at {y}", (band.spine, band.page));
                    ((band.spine, band.page), y)
                })
                .collect()
        };
        let mut previous = frame(&strip);
        for _ in 0..400 {
            strip.scroll_by(7.3);
            let current = frame(&strip);
            let moved: Vec<f32> = current
                .iter()
                .filter_map(|(id, y)| {
                    let (_, before) = previous.iter().find(|(pid, _)| pid == id)?;
                    Some(y - before)
                })
                .collect();
            assert!(
                moved.windows(2).all(|pair| pair[0] == pair[1]),
                "bands moved by different amounts: {moved:?}"
            );
            previous = current;
        }
        assert!(
            strip.scroll_y() > 2000.0,
            "the loop scrolled through a seam"
        );
    }

    #[test]
    fn sub_pixel_scrolls_add_up_and_the_frame_moves_a_pixel_at_a_time() {
        let mut strip = Strip::new(vec![4000], VIEWPORT, 1.0, 0.5, 1);
        strip.measure(0, vec![extent(0.0, 2000.0, 0)]);
        assert!(!strip.scroll_by(0.4), "0.4 px reaches no pixel: no redraw");
        assert!(near(strip.scroll_y(), 0.4), "but the fraction is kept");
        assert!(near(strip.view_top(), 0.0));
        assert!(strip.scroll_by(0.4), "0.8 rounds up: the frame moves");
        assert!(near(strip.view_top(), 1.0));
        assert!(
            !strip.scroll_by(0.4),
            "1.2 rounds back down to the same pixel"
        );
        assert!(near(strip.view_top(), 1.0));
        assert!(strip.scroll_by(0.4));
        assert!(near(strip.scroll_y(), 1.6));
        assert!(near(strip.view_top(), 2.0));
    }

    #[test]
    fn the_grid_is_the_device_pixel_at_any_scale() {
        // At 2×, a device pixel is half a CSS px: 642.6 CSS px is 1285.2
        // device px and the band grows to 1286 of them; a 3.4 px gap is
        // 6.8, so 7 — 3.5 CSS px.
        let mut strip = Strip::new(vec![4000, 4321], VIEWPORT, 2.0, 0.5, 1);
        strip.measure(0, vec![extent(0.0, 642.6, 0), extent(3.4, 428.4, 2000)]);
        let bands = strip.visible_bands();
        assert!(near(bands[0].height, 643.0), "{}", bands[0].height);
        assert!(
            near(bands[1].top, MARGIN_TOP + 643.0 + 3.5),
            "{}",
            bands[1].top
        );
        strip.scroll_by(0.3);
        assert!(near(strip.view_top(), 0.5), "0.6 device px rounds to one");
        // A guess is on the grid too: 4321 chars at the learned density
        // (1161.27 px) land on a half pixel or a whole one, never between.
        let guess = strip.slots[1].height;
        assert!(near((guess * 2.0).fract(), 0.0), "{guess}");
        // And a rebuild at 1× moves the grid with it.
        strip.rebuild(2, VIEWPORT, 1.0);
        strip.measure(0, vec![extent(0.0, 642.6, 0)]);
        assert!(near(strip.visible_bands()[0].height, 643.0));
        assert!(
            near(strip.view_top(), 0.0),
            "0.3 CSS px rounds to none at 1×"
        );
    }
}
