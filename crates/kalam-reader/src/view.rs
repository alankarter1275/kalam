...
impl ReaderView {
    ...
    pub fn connect_image_tap(&self, f: impl Fn(u32, u32, &[u8]) + 'static) {
        ...
        for spine in 0..chapter_count {
            if let Some(text) = s.cached_unit_text(spine) {
                ...
                for (byte_idx, _) in text_lower.match_indices(&query_lower) {
                    ...
                    let start_char = (char_offset as usize).saturating_sub(30);
                    let end_char = end_offset as usize + 40;
                    // Reuse the already-collected chars — no per-match allocation.
                    let snippet_chars: String = chars[start_char..end_char.min(chars.len())].iter().collect();

                    let locator = LayeredLocator {
                        spine_href: s.spine_href(spine).unwrap_or_default(),
                        spine_index: spine,
                        char_offset,
                        locator_version: chapbook_core::LOCATOR_VERSION,
                        quote: chapbook_core::Quote {
                            prefix: String::new(),
                            exact: query_trimmed.to_string(),
                            suffix: String::new(),
                        },
                        spine_fraction: if chars.is_empty() { 0.0 } else { char_offset as f64 / chars.len() as f64 },
                        book_progression: if chapter_count == 0 { 0.0 } else { (spine as f64 + (char_offset as f64 / chars.len().max(1) as f64)) / chapter_count as f64 },
                    };
        ...
        if self.mode() == ReadingMode::Paged && self.area.width() > 900 {
            if action == Action::NextPage {
                ...
                let outcome = if page >= 2 {
                    ...
                    s.set_position(spine, 0);
                    ActionOutcome::Changed
                } else if spine > 0 {
                    let prev_spine = spine - 1;
                    let prev_count = s.page_extents(prev_spine).len();
                    let target_page = if prev_count > 1 { (prev_count - 1) / 2 * 2 } else { 0 };
                    s.set_position(prev_spine, target_page);
                    ActionOutcome::Changed
                } else {
                    ActionOutcome::Unchanged
                };
        ...
        if self.mode() == ReadingMode::Paged && self.area.width() > 900 {
            ...
            if x >= single_w {
                if page + 1 < page_count {
                    Some((spine, page + 1, x - single_w, y))
                } else {
                    None
                }
            } else {
    ...
    fn widget_rects(&self, s: &mut Session, spine: usize, start: u32, end: u32) -> Vec<Rect> {
        ...
        match strip.as_ref() {
            ...
            None => {
                if self.mode() == ReadingMode::Paged && self.area.width() > 900 {
                    ...
                    for rect in s.range_rects_on_page(spine, page_left, start, end) {
                        rects.push(rect);
                    }
                    if page_right < page_count {
                        for mut rect in s.range_rects_on_page(spine, page_right, start, end) {
                            rect.origin.x += single_w;
                            rects.push(rect);
                        }
                    }
                    rects
                } else {
                    s.range_rects(start, end)
                }
            }
        }
    }
            ...
            let pixmap = match view.mode() {
                ReadingMode::Paged => {
                    ...
                    if width > 900 {
                        ...
                        if let (Some(out), Some(handles)) = (pixmap.as_mut(), view.place_handles(&mut s)) {
    ...
    fn draw_scrolled(&self, metrics: PageMetrics) -> Option<chapbook_reader::tiny_skia::Pixmap> {
        ...
        match strip_slot.as_mut() {
            ...
            Some(strip) => {
                ...
                strip.rebuild(generation, viewport, metrics.dpi_scale);
        ...
        //
        // Everything is placed from `view_top`, the scroll snapped to the
        // device-pixel grid once for the frame, against band edges the
        // strip keeps on that grid — so the rounding below never has a
        // fraction to decide, and two bands can never round apart (the
        // one-pixel shimmer at page seams under a touchpad; `scroll.rs`).
        ...
        drop(strip_slot);
        if let Some(handles) = self.place_handles(&mut s) {
            handles::paint(
                &mut out,
                &handles,
    ...
    fn install_pointer(&self) {
        ...
        click.set_button(1);
        let multi_click = Rc::new(Cell::new(0i32));
        {
            let view = self.clone();
            let multi_click = multi_click.clone();
            click.connect_pressed(move |_, n_press, x, y| {
                multi_click.set(n_press);
                if n_press == 2 {
                    let (x, y) = (x as f32, y as f32);
                    let mut s = view.inner.session.borrow_mut();
                    let ok = match view.band_at(y) {
                    ...
                    let ok = match view.band_at(y) {
                        Some((band, py)) => {
                            s.select_paragraph_at_page(band.spine, band.page, x, py)
                        }
                        None if view.mode() == ReadingMode::Scrolled => false,
                        None => {
                            if let Some((spine, page, px, py)) = view.paged_point(&mut s, x, y) {
        ...
        let grabbing = Rc::new(Cell::new(false));
        {
            ...
            drag.connect_drag_begin(move |_, x, y| {
                ...
                if let Some((w, h, rgba)) = image {
                    drop(s);
                    if let Some(cb) = &view.inner.callbacks.borrow().image_tap {
                        cb(w, h, &rgba);
                    }
                    return;
                }
        ...
        }
        {
            ...
            drag.connect_drag_end(move |gesture, dx, dy| {
                ...
                let press_count = multi_click.replace(0);
                if press_count >= 2 {
