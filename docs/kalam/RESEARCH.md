...
## R18. Owner's comparison after steps 1+2: match (2026-09-14)

Owner's screenshot of Nyxia chapter 1 in Kalam on engine 63d854f (scroll
mode, dark theme, 1366×768) against `docs/files/webkit gtk.png`:
heading `.para-cda-alt-chap-pg.sans` sans/1.57em/left, `.para-cda1` bold
with the italic title, the ~4em gap, `.para-pf` flush, `.para-p` 1em
indent + justify, ~30 px leading — all identical in structure. Steps 1
and 2 achieved the PLAN §10 reading of "Kalam's theme wins": colours,
body face, size, leading are Kalam's; everything else is the book's.

Residual differences, none a defect of the cascade:
- Body face: engine = bundled Literata; WebKit showed a system serif
  fallback (the old skin listed Literata first, so the owner's machine
  has no Literata installed — WebKit fell through the list). Literata is
  wider, so ~1 line less per screen and looser justified lines (no
  `hyphens: auto` in the book's CSS, so the engine does not hyphenate).
- Heading starts ≈20 px lower than WebKit's. `div.page_top_padding
  { margin-top: 10% }` resolves against the column width (≈62 px at
  620 px) in both engines, so the difference is the widget's top inset
  vs. the old `body { padding-top: 56px }`. Check `kalam-reader`'s page
  margins during step 3; cosmetic either way.
- The page pill overlaying the bottom lines is Kalam's overlay.

Next: step 3 (scroll-mode ripple: text below the first screen shifts as
height estimates are corrected).

## R19. Handoff hygiene: the drop-in went stale, and an update was under-described (2026-09-14)

Two corrections from the calibre-alt agent after the 63d854f bundle.

1. **`patch/engine.rs` stale, third round running.** It still had the
   `connect_word` tap-to-dictionary call the owner removed, the 320 px
   card, none of the chip work. It was the seed Kalam's
   `src/pages/reader/engine.rs` grew from; the moment the other agent
   edited theirs, any copy here could only be stale — and a stale
   drop-in that the recipe says to "copy to src/pages/reader/engine.rs"
   is a trap for the next agent. Retired: `make-bundle.sh` no longer
   ships it; AGENT-BRIEF says Kalam's copy is authoritative and the
   API-reference diff between bundles is the change list; INTEGRATION
   carries a status note; the seed stays under `docs/kalam/patch/` with
   a header saying what it is. Rule: a handoff file the other side
   *edits* is shipped once, then dropped.

2. **"formatting fix; no API changes; rebuild only" was accurate about
   the API and too small about behaviour.** The range befb91e..63d854f
   turns publisher stylesheets on for every book and changes which
   parser most books get. Both were intended and both are visible on
   every page. The other side reads the commit range; describe every
   Kalam-visible change in the reply, API or not (WORKING §7a updated).

## R20. The scroll-mode "ripple": per-band rounding under a touchpad (2026-09-14)

Step 3 of the formatting task. Owner's description, on the 63d854f build:
a hair-width wobble — "text below some point jiggles by about a pixel as
I scroll; the part above stays still" — with a **touchpad**, from the
first scroll and all through the book. Not a jump, not a settle.

**Mechanism (found by reading `draw_scrolled` and porting `Strip` to a
Python model).** A frame placed each visible band at
`dst_y = round((band.top − scroll_y) · scale)`, *per band*. Band tops are
sums of measured page heights and gaps, which are almost never whole
pixels (21 lines × 30.6 px = 642.6; a discarded margin of 3.4), and a
touchpad moves the scroll by fractions of a pixel per event. As the
scroll's fraction sweeps, the two bands on screen cross their rounding
thresholds at *different* scroll values, so for part of every pixel of
travel the lower band sits one pixel closer to or further from the upper
one than it should. From the first screen on, a page seam is always in
view, so it is always visible. The model reproduced it exactly: at 7.3 px
per event, 1172 of 3000 frames had the two bands move by different
amounts; at a whole-pixel wheel notch (92 px) none — the fraction never
changes — which is why the round-H verification (wheel) saw nothing; at
PageDown (0.9 × viewport = 666.9 px) 7 of 3000. Measurements (estimate
corrections) were *not* the cause: both bands and the scroll shift by the
same amount, so the anchor arithmetic was already rigid, as its tests
said.

**Fix (`kalam-reader/src/scroll.rs`, `view.rs`).** Put the strip on the
device-pixel grid and draw every frame from one snapped origin:
- `Strip` learns the widget's `scale`; `measure()` snaps each page's
  `used_height` *up* to the next device pixel (no row lost) and
  `gap_before` to the nearest; `reflow()` snaps every guessed height.
  With `MARGIN_TOP`/`CHAPTER_GAP` whole, every band edge is then on the
  grid.
- New `Strip::view_top()` = `scroll_y` snapped to the grid. Painting,
  `visible_units/bands/dividers`, `widget_to_page` and `to_widget` all use
  it, so paint, hit-testing and handles agree to the pixel. The raw
  `scroll_y` keeps its fractions (the scrollbar shows it; the anchor is
  taken from it), so slow touchpad motion accumulates instead of being
  rounded away per event.
- `set_scroll` now reports whether the *frame* moved; a sub-pixel change
  queues no redraw.
With every edge and the origin on the grid, `band.top − view_top` has no
fraction to round, and between two frames every band moves by the same
whole number of pixels. The model: 0 of 3000 frames at every step size.
Three new unit tests pin it (`fractional_scrolling_moves_every_band_by_
the_same_whole_pixel`, `sub_pixel_scrolls_add_up_…`,
`the_grid_is_the_device_pixel_at_any_scale`).

**Also checked (R18 residual).** The heading sitting ≈20 px lower than
WebKit's is not the widget's inset: `MARGIN_TOP` is 48 px against the old
`body { padding-top: 56px }`, so the widget is 8 px *tighter*. Left as
cosmetic; the difference is somewhere in how the two engines treat
`div.page_top_padding { margin-top: 10% }` at the top of the first page.

## R21. Images behind inline wrappers were dropped (2026-09-15)

Owner's report on the a745d5e build of Nyxia (PRH EPUB 3): the full-page
"PART I / BROKEN THINGS" plate (spine `p001`) is a blank page between the
divider and chapter 1; `p002` and `s001` have the same shape. The pixels
are not the problem: all 24 JPEGs are baseline 3-component, the same
kind that renders elsewhere in the book, and `collect_images` decodes
per `<img>` node without looking at where the node sits.

**Mechanism (boxtree, inherited).** `build_block` decided "container of
blocks" versus "one inline formatting context" from *direct* children
only: a replaced child (`replaced_kind`) or a block-level child made a
container; otherwise the whole subtree was flattened into one inline run
by `collect_inline`, whose element branch (`collect_inline_element`)
knows `<br>` and generated content and nothing else. `collect_container`
sent inline children the same way. `display_of` classes `inline-block`
as `Inline`. So an `<img>` reached through *any* inline element — a link,
a span, PRH's `div.squeeze {display:inline-block}` sizing wrapper — was
never turned into a block and vanished. Upstream documents the limit as
"images nested deeper than one level inside inline content are skipped";
the hand trace says depth one is lost too (`div.figure_cover(block) >
div.squeeze(inline-block) > img`: `has_block_child(figure_cover)` is
false, so its subtree is one inline run and the image is gone). The owner
reported the cover as rendering; that was not reconciled from the sandbox
(Kalam may draw the cover itself, or the trace misses something) and it
does not matter for the fix, which is depth-independent. Nyxia's shapes,
from the XHTML:

| Shape | Files |
|---|---|
| `div.figure_cover > div.squeeze > img` | `cvi` |
| `div.figure_title > div.squeeze > img` | `tp` |
| `div.figure_medium.sans > div.squeeze > img` | 13 chapter figures |
| `div.figure_fullpage > div.squeeze > img` | 5 leaderboard plates |
| `div > div.squeeze.center > div.squeeze.squeeze95 > img` | `p001`, `p002`, `s001` (the blank ones) |

**Fix (kalam, `boxtree.rs`).** `has_block_child` became a function with
one more rule: an inline child that *wraps* a placeable replaced element
counts as block-level. `wraps_replaced` walks the displayed descendants
of an inline element and is true when one is `replaced_present` — an
`<img>` whose tag has dims in the `ImageStore` (it decoded), an `<hr>`, a
rasterized `<svg>`, a prepared formula — or itself wraps one.
`collect_container` sends such a wrapper to `build_block` instead of
`collect_inline_element`, so one level down the image is a direct child
and hoists exactly as `<p><img/>text</p>` always did; text inside the
wrapper becomes anonymous blocks above and below it (upstream's own
flattening, now at any depth). Unchanged on purpose: inline wrappers
holding only text (PRH's `span.list_ornament`, `inline-block` or not)
stay in their line; an `<img>` that failed to decode or is
`display: none` makes no wrapper a block, so its text stays inline as
before. A floated image behind a link now reaches `place_float_image`.
Six tests in `tests/pagination.rs` under "replaced elements inside inline
wrappers"; the first is the exact `p001` shape.

**Effect on goldens.** None expected: the illustrated fixture's images
are direct children of `<section>`/`<p>`, the foreign fixture's `<img>`,
`<svg>` and `<math>` likewise; no fixture has a replaced element inside
an inline element (checked with a scan of every `.epub` under
`fixtures/`). CI decides; if a golden moves, the mechanism above is what
to check against before re-baselining.

**Left open from the same report.** (1) Dark themes: the old WebKit skin
inverted images (`filter: invert(1) brightness(1.06) contrast(1.04);
mix-blend-mode: screen`), sparing covers/photos/figures by class, id or
src; the engine paints every image as-is, so a white-paper plate is a
white rectangle on a dark page. `KalamTheme::is_dark()` says "Night
themes invert images" and nothing implements it. Design: decide the mode
in `build_display_list` from the background's luma, carry it on
`DisplayOp::Image`, apply in `draw_image`; `illustrated-s0p0-dark.png`
will change and is regenerated by the owner. (2) The scroll wobble
survived the pixel-grid fix (R20): the model reproduced the old symptom,
not the remaining one, so what remains is a mechanism the model did not
have — candidates are the cairo `ImageSurface` painted under
`ctx.scale(1/scale)` in `install_draw` (device↔logical truncation),
whole-page `render_page` per band per frame, and GTK's own texture
placement. A screen recording committed under `docs/files/` would settle
which.

## R22. Pictures on a dark page: paper inverts, pictures do not (2026-09-15)

Owner's report on the a745d5e build of Nyxia in Kalam's Dark theme: the
part-title plates and chapter ornaments — black lettering on white,
shipped as JPEGs — sat on the page as bright white slabs, where the old
WebKit reader showed light lettering on the dark ground.

### What the old reader did

`reading_css()` in calibre-alt `src/epub_book.rs` (main), night themes:

```css
img, svg, image { filter: invert(1) brightness(1.06) contrast(1.04) !important;
...
img[src*="photo" i], figure > img[alt]:not([alt=""]) {
  filter: brightness(0.86) !important; mix-blend-mode: normal !important; }
```

and on Light/Sepia `mix-blend-mode: multiply` for every image, with the
same cover/photo exceptions. Author wrappers got
`background-color: transparent !important` so the blend composited
against the page colour.

Two observations about that rule. First, invert-then-screen is exactly
right for a plate: black ink → white, white ground → black, and screen
makes black take the page colour, so the plate reads like the text
around it with no rectangle. Second, the exception list is a guess about
pixels made from file names and class names, and on Nyxia it guesses
wrong both ways. The cover's `<img>` has `class=""` and no `id` — the
`figure_cover` class and `id="cover"` are on wrapper `div`s — and its
file is `…_001_cvi.jpg`, so no exception matched and the old reader
inverted the cover. The plates carry `alt` text, so had PRH wrapped them
in `<figure>` instead of `<div>`, `figure > img[alt]` would have
*excluded* them from inversion — the one thing the rule exists to do.

### What the engine does

The pixels are in hand, so the decision is made from them.
`ImageLook::of_rgba` (`crates/chapbook-paint/src/images.rs`) samples
about 4 k pixels of the straight RGBA at `ImageStore::insert` and
classifies:

- *light*: Rec. 601 luma ≥ 176, or alpha < 128 (the page shows through);
- *vivid*: chroma (max − min channel) ≥ 64;
- *tones*: the number of 4-wide luma bins (of 64) holding ≥ 1 % of the
  opaque, unvivid samples.

`Paper` = light ≥ 60 % ∧ vivid ≤ 5 % ∧ tones ≤ 10; else `Picture`. The
tones measure is what separates a high-key photograph from a plate:
ink on paper is a few spikes (ground, ink, maybe a grey fill) with the
anti-aliased edges too thin to register, while continuous tone lights
up a run of bins even at 78 % white. Measured (Python port of the same
arithmetic, ImageMagick decode):

| image | light | vivid | tones | verdict |
|---|---|---|---|---|
| Nyxia plates ×23 (`_001`–`_023_r1.jpg`) | 0.90–0.99 | 0.00 | 1–5 | Paper |
| Nyxia cover (`_001_cvi.jpg`) | 0.02 | 0.18 | 20 | Picture |
| B&W executive portraits ×3 (+ brightened copies) | 0.46–0.78 | 0.00 | 12–22 | Picture |
| scanned engravings ×4 | 0.60–0.84 | 0.00 | 17–43 | Picture |
| colour diagram on white | 0.88 | 0.12 | 2 | Picture (vivid) |
| grey-on-white table, line drawing, cream letter | 0.92–1.00 | 0.00 | 1–6 | Paper |
| `square.png` (illustrated fixture) | 0.00 | 1.00 | — | Picture |
| white-on-black plate | 0.00 | 0.00 | 1 | Picture (not light) |
| black text on *transparent* PNG (anti-aliased) | 1.00 | 0.00 | 64 | Picture |

The last row is the known miss: a transparent-background PNG whose
anti-aliased edges are the only opaque unvivid pixels has every tone
bin over 1 % of that tiny opaque set. It paints plain (dark text over
the page, i.e. invisible on a dark ground) — the same as before this
change, so nothing regressed; fixing it means weighting `tones` by the
opaque fraction. Not done: no such image in the books at hand.

`ImageTreatment` (`display.rs`) is chosen per op in
`build_display_list`, which now takes the `ImageStore`:

| ground | Paper | Picture / not in store |
|---|---|---|
| white exactly | Plain | Plain |
| light (luma ≥ 128) | Multiply | Plain |
| dark | Invert (negative + screen) | Plain |

Pure white is `Plain` because multiply over `d = 1` *is* source-over
(`s·(1−da) + d·(1−sa) + s·d = s + (1−sa)`) in both of tiny-skia's
pipelines (`highp.rs:428`, `lowp.rs:487`), so the light render goldens
are untouched and the CLI's white page does no extra work. On Kalam's
Light paper (`#faf8f5`) and Sepia (`#f5f0e8`) a plate's white takes the
tint, as it did under WebKit's `multiply`. Pictures are never
multiplied: the old reader did tint photographs on sepia, but a tinted
photograph is a worse photograph, and the exception list was trying to
say so anyway.

Inversion on premultiplied pixels is `c' = a − c` per channel (stays
premultiplied; transparent stays transparent). It happens once per
store: `layout_text_unit` calls `ImageStore::negate_paper()` when the
palette's ground is dark, so the renderer screens plates straight from
the store; `draw_image` falls back to a per-draw `inverted_copy` when
the stored polarity is not the one the treatment wants (a light session
never negates, so `Multiply`/`Plain` also draw in place). The store is
per unit and rebuilt on any settings change (`drop_metrics_dependent`
clears EPUB image stores), so switching theme re-decodes and re-judges.
`is_dark_ground` (luma < 128) is now the one definition shared by the
selection band's blend and the image treatment.

### Cost

`of_rgba` is ~4 k samples per image, once per decode — noise next to
JPEG decoding. `negate_paper` is one pass over each paper image's bytes,
once per decode, dark ground only. Per frame, nothing new: the
treatment is a `PixmapPaint.blend_mode`, and the pixels drawn are the
stored ones.

### Verification

Unit tests in `images.rs` (thresholds, transparency, idempotent
negation), `display.rs` (the treatment table, unknown resources),
`render-tinyskia` (inverted plate screened onto `#1b1e24` reads
(255,255,255) ink / (0x1b,0x1e,0x24) ground; multiply over white equals
source-over byte for byte; negated store draws plain correctly), and
two session tests over `fixtures/epub/plates.epub` — a new fixture with
a PRH-shaped plate (`div.squeeze.center > div.squeeze.squeeze50 > img`,
120×60, ink bar rows 24–36) and a colour gradient picture — that check
the pixels on white, sepia and dark grounds. `illustrated-s0p0-dark.png`
is unchanged because `square.png` is a Picture; the light goldens
because white is `Plain`.

Not verified here (no display): what the plates look like on the
owner's screen. The mechanism matches the CSS the owner accepted for a
year, so the expectation is "as before, without the cover inverted".
