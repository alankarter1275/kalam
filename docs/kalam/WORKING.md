...
fixtures/epub/              long.epub (8 ch), minimal.epub, illustrated.epub, plates.epub (OURS: a PRH-shaped text plate + a colour picture, for the dark-theme image rules)
docs/kalam/                 PLAN (why), WORKING (this), RESEARCH (findings), UPSTREAM (fork hygiene), INTEGRATION (Kalam recipe, executed), patch/ (the seed engine.rs Kalam grew from — history; Kalam's copy is authoritative)
...
- **The strip lives on the device-pixel grid** (R20): measured page
  heights are snapped up, gaps and guesses to the nearest pixel, and
  every frame is drawn from `Strip::view_top()` — the scroll snapped
  once. Do not round per band again (a touchpad's fractional scrolling
  made the text below every page seam shimmer by a pixel), and do not
  snap `scroll_y` itself (sub-pixel touchpad motion must accumulate).
...
`docs/engine-handoff/`): the recipe, a source-generated API reference
(`api-reference.py`), the widget README and the engine commit. (The
drop-in `engine.rs` was retired from the bundle on 2026-09-14 — Kalam's
copy had diverged from it three rounds running; see R19.) Protocol for the other agent: `AGENT-BRIEF.md`;
...
<commit>; regenerate bundle" — and list every change *visible from
Kalam* (page text, behaviour), not only API changes: the other side
reads the commit range and will, rightly, flag an update described as
smaller than it is (R19). Record anything learned about Kalam's
