...
# No patch/engine.rs any more (2026-09-14). That file seeded Kalam's
# src/pages/reader/engine.rs once; Kalam has edited its copy every round
# since, so a copy here can only be stale, and a stale drop-in beside a
# live file is a hazard. Kalam's tree is authoritative; the seed stays in
# docs/kalam/patch/ as history only.
